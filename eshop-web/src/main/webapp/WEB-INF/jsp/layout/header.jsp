		...
		<div class="container">
			...
			<div class="navbar-collapse collapse">
				<ul class="nav navbar-nav">
					<li class="${param.page eq 'items' ? 'active' : ''}"><a
						href="${homeUrl}">Items</a></li>
					<c:url var="basketUrl" value="/basket.html" />
					<li class="${param.page eq 'basket' ? 'active' : ''}"><a
						href="${basketUrl}">Basket</a></li>
					<c:url var="orderUrl" value="/order.html" />
					<li class="${param.page eq 'order' ? 'active' : ''}"><a
						href="${orderUrl}">Order items!</a></li>
					<c:url var="userOrdersUrl" value="/user-orders.html" />
					<li class="${param.page eq 'user_orders' ? 'active' : ''}"><a
						href="${userOrdersUrl}">User orders</a></li>
					<c:url var="aboutUrl" value="/about.html" />
					<li class="${param.page eq 'about' ? 'active' : ''}"><a
					...
					<c:choose>
						<c:when test="${loggedIn}">
							...
							<li class="${param.page eq 'logout' ? 'active' : ''}"><a
								href="${logoutUrl}">Logout ${pageContext.request.remoteUser}</a></li>
						</c:when>
						<c:otherwise>
							<c:url var="registerUrl" value="/register.html" />
							<li class="${param.page eq 'register' ? 'active' : ''}"><a
